#' ---
#' title: "Zygotic Embryogenesis first degree neighbors"
...
#' * Data
edgelist <- read.table(here("data/seidr/edgelist/backbone-edgelist.txt"),
...
#' From the differential expression results, we can load our DE genes (_i.e._ our genes of interest)
goi <- sub("\\.1","",read.csv(here("data/DE/DE-genes.csv.gz"),as.is = TRUE)[,1])
...
write_graph(fdn,format = "graphml",
            file=here("data/seidr/graphml/firstDegreeNeighbour.graphml"))
